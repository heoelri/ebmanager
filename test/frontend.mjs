...
assert.match(javascript, /<details id="pendingDivera" class="card resource-section" hidden aria-live="polite"><summary>DIVERA Import<\/summary>/);
assert.doesNotMatch(javascript, /<details id="pendingDivera"[^>]*\sopen(?:\s|>)/);
...
assert.equal(location.search,'?incident=42');
assert.deepEqual(requested,['resources','incident:42','home','incident:42']);
await navigation.navigate('unknown');
assert.equal(location.search,'?view=home');
assert.deepEqual(requested,['resources','incident:42','home','incident:42','home']);
await navigation.navigate('admin');
assert.equal(location.search,'?view=home');
assert.deepEqual(requested,['resources','incident:42','home','incident:42','home','admin']);
