...
Der PR-Kommentar läuft getrennt über `workflow_run` mit der unveränderlich vom Standardbranch geladenen Workflowdefinition. Nur erfolgreiche Screenshot-Läufe für Quell-Branches desselben Repositorys werden verarbeitet. Dieser zweite Workflow lädt ausschließlich das erzeugte Artefakt, führt keinen PR-Code aus und veröffentlicht einen Kommentar mit Artefaktlink und Dateiliste. GitHub-Medienanhänge werden nicht hochgeladen, weil der automatische `GITHUB_TOKEN` dafür kein unterstützter Authentifizierungstyp ist. GitHub CLI 2.99.0 wird mit fester Version und SHA-256-Prüfsumme geladen; ein persönlicher Zugriffstoken oder Deployment-Secret wird nicht verwendet. Fork-PRs erreichen den schreibenden Job nicht und erhalten weiterhin ausschließlich das Artefakt.
