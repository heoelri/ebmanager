...
- Änderungen unter `public/**` oder am Screenshot-Test erzeugen mit dem lokalen Demo-Profil und einer gepinnten Playwright-Version 16 UI-Screenshots aller rollenabhängigen Hauptansichten als Workflow-Artefakt. Ein separater `workflow_run` auf dem Standardbranch verlinkt und listet sie nur für PR-Branches dieses Repositorys in einem aktualisierten Kommentar; verwende dafür niemals `pull_request_target`, Medienanhänge oder einen persönlichen Zugriffstoken im PR-Workflow.
