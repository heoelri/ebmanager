...
- Ab dem allerersten Einheitsbericht bleiben gemeinsame Einsatzdaten und alle bereits vorhandenen Einheits-Fahrzeug-Snapshots erhalten, auch für alarmierte Einheiten ohne Bericht. `incidents.report_data_frozen` wird unter derselben Einsatzsperre gesetzt wie die Berichtsanlage. Spätere DIVERA-Abweichungen werden nur mit Einsatz-ID und Feldkategorien sichtbar gewarnt; neue Einheiten dürfen ihren ersten Snapshot erhalten. Es gibt keine allgemeine Korrektur-/Revisionsfunktion.
- `reports.author_name` und `report_crew.member_name` sind historische Berichtsdaten. Bestehende Besatzung behält ihren Namen auch beim Umverteilen; neu aufgenommene Personen erhalten ihren aktuellen serverseitigen Namen. Ansichten, Zusammenfassungen und PDFs verwenden die Snapshots, Ressourcen weiterhin Stammdaten. Der Prüfverlauf behält seine unveränderlichen `actor_name`-Werte.
- Die mandantengleiche Autorenzuordnung bleibt Voraussetzung der Berichtslesesicht. Ein leerer Namensbackfill repariert keine ungültige Altzuordnung und darf Berichte mit mandantenfremdem Autor nicht freigeben.
...
- Die Statistik steht Einheits- und Wehrführungen zur Verfügung. Einheitsführungen sehen ausschließlich ihre aktuell zugeordnete Einheit; Wehrführungen sehen die gesamte Organisation und können nach Einheit filtern. Wehrweite Einsatzzahlen zählen einen mehreren Einheiten zugeordneten Einsatz einmal, der Einheitenvergleich zählt seine jeweilige Zuordnung. Alarmierte Fahrzeuge der eigenen Einheit stammen aus `incident_units.vehicles`; dort als fremd markierte Fahrzeuge werden nicht gezählt. Tatsächliche Beteiligung stammt aus `report_crew` und zusätzliche Fahrzeuge aus `report_additional_vehicles`; Zeitkategorien werden mit den Grenzen aus `constants.php` in `Europe/Berlin` berechnet.
...
- `incidents.is_exercise` kennzeichnet einen manuellen oder importierten Einsatz organisationsweit als Übung und bleibt bei DIVERA-Importen unverändert. Wehrleitung darf organisationsweit ändern, Einheitsführung und Führungskräfte bei Zuordnung zu mindestens einer beteiligten Einheit. Änderungen sind einsatzrevisionsgeschützt, verändern keine Berichte und werden mit altem/neuem Wert, Akteurssnapshot und UTC-Zeitpunkt in `incident_exercise_changes` protokolliert. Übersicht, Berichte und PDFs kennzeichnen Übungen; die Statistik weist Übungen und reguläre Einsätze separat aus, enthält beide aber gemeinsam in den übrigen Kennzahlen.
...
- `vehicles` enthält den aktuellen einheitsspezifischen Fahrzeugstamm aus `cluster.vehicle`; `incident_units.vehicles` bleibt ab dem ersten Bericht ein unveränderlicher Einsatz-Snapshot. Zusätzliche eigene Fahrzeuge eines Berichts liegen getrennt in `report_additional_vehicles`.
...
- Ein DIVERA-Einsatz ist innerhalb einer Organisation über `divera_id` eindeutig. Wiederholter Import aktualisiert Einsatz und bestehende `incident_units` nur vor dem ersten Bericht bei echten Änderungen; die Reihenfolge einer Fahrzeugliste allein ist keine Änderung. Das Importprotokoll darf trotzdem jeden erfolgreichen Import notieren.
...
- Neue Berichte verwenden für Ausrücken, Eintreffen und Einsatzende getrennte native Datums- und Uhrzeitfelder: Datum vorbelegt mit dem lokalen Alarmtag, Uhrzeit leer. Ein gemeinsames Datum aktualisiert nur Datumswerte, die noch dem bisherigen gemeinsamen Datum entsprechen; individuelle Abweichungen bleiben erhalten. Die Alarmierung bleibt unveränderlich, gespeicherte Berichte behalten ihre bisherigen Zeitfelder.
- Asynchrone UI-Antworten gelten nur für ihre noch aktuelle Ansicht, ihren Dialog und gegebenenfalls ihre Einheitsauswahl. Verworfene Antworten lösen weder Folgeaktionen noch verspätete Fehler im neuen Kontext aus; bereits gesendete Schreibvorgänge werden nicht automatisch wiederholt. Warnungen bleiben direkt beim auslösenden Vorgang, ohne globalen Puffer.
- Übungsänderungen erhalten offene Formularknoten und aktualisieren ausschließlich die Kennzeichnung, deren Verlauf und die durch den eigenen Schreibvorgang erhöhte Einsatzrevision. Besatzungsformulare verwenden ihre bereits geladenen Ressourcen bei Zusatzfahrzeugänderungen erneut; es gibt keine persistenten Kopien von Formularinhalten.
...
- Alle Pull Requests und Pushs auf `main` führen die Browserprüfungen einschließlich des echten rollenübergreifenden Berichtsworkflows aus und erzeugen mit dem lokalen Demo-Profil und einer gepinnten Playwright-Version insgesamt 17 UI-Screenshots aller rollenabhängigen Hauptansichten als Workflow-Artefakt. Ein separater `workflow_run` auf dem Standardbranch schreibt sie für PR-Branches dieses Repositorys in einen ersetzten Screenshot-Branch je PR und bettet unveränderliche Raw-URLs direkt in einen aktualisierten Kommentar ein; verwende dafür niemals `pull_request_target`, User Attachments oder einen persönlichen Zugriffstoken im PR-Workflow.
- Coverage-Artefakte unterscheiden PHP-Zeilen der HTTP-Smoke-Requests von benannten Browserfunktionen und dort echte Backend- von gemockten Lifecycle-Tests. Direkte CLI-/Node-Prüfungen und Zweigabdeckung werden nicht als mitgemessen ausgegeben; fehlende Messdaten sind kein Erfolg. Xdebug bleibt eine CI-/lokale Testabhängigkeit ohne Produktionskonfiguration. PHP-Assertions müssen aktiv sein.
